...
After the above is done your project will be configured with the base airbnb config. In order for eslint to not get angry about certain conventions that Next follows (such as how Links work) we will have to make a few modification to the eslintrc file: 
...
Now we will create two config files: 
...
Only one more config file (I promise) 🎉
