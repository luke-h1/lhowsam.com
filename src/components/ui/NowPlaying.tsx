import React, { useSyncExternalStore } from 'react';
import spotifyService from '../../services/spotifyService';
import type { Song } from '../../types/spotify';
import styles from './NowPlaying.module.css';

const POLL_MS = 3000;

const spotifyLogoPath =
  'M83.996.277C37.747.277.253 37.77.253 84.019c0 46.251 37.494 83.741 83.743 83.741 46.254 0 83.744-37.49 83.744-83.741 0-46.246-37.49-83.738-83.745-83.738l.001-.004zm38.404 120.78a5.217 5.217 0 01-7.18 1.73c-19.662-12.01-44.414-14.73-73.564-8.07a5.222 5.222 0 01-6.249-3.93 5.213 5.213 0 013.926-6.25c31.9-7.291 59.263-4.15 81.337 9.34 2.46 1.51 3.24 4.72 1.73 7.18zm10.25-22.805c-1.89 3.075-5.91 4.045-8.98 2.155-22.51-13.839-56.823-17.846-83.448-9.764-3.453 1.043-7.1-.903-8.148-4.35a6.538 6.538 0 014.354-8.143c30.413-9.228 68.222-4.758 94.072 11.127 3.07 1.89 4.04 5.91 2.15 8.976v-.001zm.88-23.744c-26.99-16.031-71.52-17.505-97.289-9.684-4.138 1.255-8.514-1.081-9.768-5.219a7.835 7.835 0 015.221-9.771c29.581-8.98 78.756-7.245 109.83 11.202a7.823 7.823 0 012.74 10.733c-2.2 3.722-7.02 4.949-10.73 2.739z';

function AnimatedBars() {
  return (
    <div className={styles.bars} aria-hidden>
      <span />
      <span />
      <span />
    </div>
  );
}

type Snapshot = {
  data: Song | null;
  failed: boolean;
};

function createNowPlayingStore() {
  const initialSnapshot = (): Snapshot => ({
    data: null,
    failed: false,
  });
  let snapshot: Snapshot = initialSnapshot();
  const listeners = new Set<() => void>();
  let intervalId: number | null = null;
  let isLoading = false;

  const emit = () => {
    listeners.forEach(listener => listener());
  };

  const load = async () => {
    if (isLoading) {
      return;
    }

    isLoading = true;

    try {
      const json = await spotifyService.lambdaNowPlaying();
      snapshot = {
        data: json,
        failed: false,
      };
    } catch {
      snapshot = {
        ...snapshot,
        failed: true,
      };
    } finally {
      isLoading = false;
      emit();
    }
  };

  const start = () => {
    if (typeof window === 'undefined' || intervalId !== null) {
      return;
    }

    load().catch(() => {});
    intervalId = window.setInterval(() => {
      load().catch(() => {});
    }, POLL_MS);
  };

  const stop = () => {
    if (intervalId === null || listeners.size > 0) {
      return;
    }

    window.clearInterval(intervalId);
    intervalId = null;
  };

  return {
    getSnapshot: () => snapshot,
    reset: () => {
      snapshot = initialSnapshot();
      isLoading = false;
      if (intervalId !== null) {
        window.clearInterval(intervalId);
        intervalId = null;
      }
      listeners.clear();
    },
    subscribe: (listener: () => void) => {
      listeners.add(listener);
      start();

      return () => {
        listeners.delete(listener);
        stop();
      };
    },
  };
}

const nowPlayingStore = createNowPlayingStore();

export function resetNowPlayingStore() {
  nowPlayingStore.reset();
}

export default function NowPlaying() {
  const { data, failed } = useSyncExternalStore(
    nowPlayingStore.subscribe,
    nowPlayingStore.getSnapshot,
    nowPlayingStore.getSnapshot,
  );

  const playing = Boolean(data?.isPlaying);
  const showCover = playing && Boolean(data?.albumImageUrl);
  let statusLabel = 'Spotify';
  let artistLabel = 'Not playing';

  if (failed) {
    statusLabel = 'Not Playing';
    artistLabel = 'Spotify';
  } else if (playing) {
    statusLabel = data?.title ?? 'Playing';
    artistLabel = data?.artist ?? 'Spotify';
  }

  return (
    <div className={styles.row} data-testid="now-playing" aria-live="polite">
      {showCover && data?.albumImageUrl !== '' && data?.songUrl && (
        <a
          className={styles.coverLink}
          href={data?.songUrl}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`Open ${statusLabel} on Spotify`}
        >
          <img
            className={styles.cover}
            src={data.albumImageUrl}
            alt=""
            width={44}
            height={44}
          />
        </a>
      )}
      {playing && data?.songUrl ? (
        <a
          className={styles.title}
          href={data?.songUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
          {statusLabel}
        </a>
      ) : (
        <p className={styles.notPlaying}>{statusLabel}</p>
      )}
      <span className={styles.sep} aria-hidden>
        –
      </span>
      <p className={styles.artist}>{artistLabel}</p>
      {playing ? (
        <AnimatedBars />
      ) : (
        <svg className={styles.spotifyLogo} viewBox="0 0 168 168" aria-hidden>
          <path fill="#1ED760" d={spotifyLogoPath} />
        </svg>
      )}
    </div>
  );
}
