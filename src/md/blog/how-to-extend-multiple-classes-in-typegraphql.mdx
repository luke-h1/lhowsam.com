...
Here we are specifying that we expect a firstName and a location (both of type sring) to be sent from the client. 
...
Here we have a function that has an input type (called OkInput). It has a single field called 'ok' which is a boolean. This function takes in a class and returns this okInput. All it does is extend from this BaseClass that you pass in. You should also note we are using a generic here in order to be type safe (we're passing the generic as the type of BaseClass).
...
Now we can reload our GraphQL dashboard and we can now see that RegisterInput is inherting the OkMixin

<Image 
alt={`picture of gql playground`}
src={`/images/gql-playground.png`}
width={420}
height={600}
priority 
/>